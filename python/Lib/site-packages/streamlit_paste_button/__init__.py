...
def _data_url_to_image(data_url: str) -> Image:
    """Convert base64 data string to a Pillow Image"""
    try:
        _, _data_url = data_url.split(";base64,")
    except ValueError:
        _data_url = data_url
    _data_url = unquote(_data_url)
    raw = base64.b64decode(_data_url)
    img_bytes = io.BytesIO(raw)
    for fmt in (None, "PNG", "JPEG", "BMP", "GIF", "WEBP"):
        try:
            img_bytes.seek(0)
            if fmt:
                return Image.open(img_bytes, formats=[fmt])
            else:
                return Image.open(img_bytes)
        except Exception:
            continue
    raise ValueError(f"无法识别的图片格式 (前8字节: {raw[:8].hex()})")
